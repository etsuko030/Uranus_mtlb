function stegnoBatch2(imfolder, exfolder)

% for B2011 data format
...
%           imwrite(s, char(strcat(one(k), '\', ID, '.png')));
    ...
    if isFolderEmpty(canLocation)==0
        ...
        for i = 1 : length(canData.Files)
            ...
            % get ID of the mask
            [~, name, ~] = fileparts(char(nowloc));
            [~, wordname, mid] = fileparts(name); % format: xxx.x.1-m-1
            mid = strrep(mid, '.', '');
            mid = strrep(mid, '-m-', '.'); % mid -> imgnum.masknum
            [~, ID, masknum] = fileparts(mid);
            newID = strrep(name, '-m', '');   % xxx-m-x  ->  xxx-x
%           disp(ID);
            % get original image by ID
            originalName = strcat(imfolder, '\', childName(k), '\', childName(k),'.', ID, '.png');
            originalImg = imread(char(originalName));
            % get mask image
