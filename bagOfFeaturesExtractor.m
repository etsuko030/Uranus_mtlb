function [features, featureMetrics] = bagOfFeaturesExtractor(I)
% [features, featureMetrics, varargout] = bagOfFeaturesExtractor(I)
...
[height,width,numChannels] = size(originalImg);
...
%
% multiscaleSURFPoints = detectSURFFeatures(greyImg);
                    
%% Step 3: Extract features
% Finally, extract features from the selected point locations. By default,
% bagOfFeatures extracts upright SURF features. 

% huMoments = invmoments(maskImg);
[hog, ~] = hogForBoW(greyImg, maskImg);

features = hog;

[ftrow, ftcol] = size(features);
% features = extractHOGFeatures(greyImg);

% features = extractFeatures(greyImg, multiscaleGridPoints,'Upright',true);

% features = extractHOGFeatures(greyImg, multiscaleGridPoints);
